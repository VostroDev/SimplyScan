...
app.whenReady().then(() => {
  ...
  protocol.handle('scan', async (request) => {
    ...
    try {
      // Read the file directly and return as a Response
      ...
      console.error('Failed to load scanned image:', error)
  ...
  ipcMain.handle('scan-page', async (_, deviceId) => {
    try {
      ...
      // URL encode the path to preserve colons and special characters
      const encodedPath = encodeURIComponent(filePath)
      return {
        success: true,
        path: filePath,
        image: `scan://${encodedPath}`
      }
    } catch (e) {
